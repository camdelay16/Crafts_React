...
const CraftForm = (props) => {
  ...
  const handleUpdateCraft = async (formData, craftId) => {
    try {
      const updatedCraft = await craftService.update(craftId, formData);
      if (updatedCraft.error) {
        throw new Error(updatedCraft.error);
      }
      const updatedCraftList = craftList.map((craft) =>
        craft._id === craftId ? updatedCraft : craft
      );
  ...
  const handleSubmit = (e) => {
    ...
    if (selectedCraft) {
      handleUpdateCraft(updatedFormData, selectedCraft._id);
      setFormData(initialState);
      navigate(`/crafts/${selectedCraft._id}`);
    } else {
      handleAddCraft(updatedFormData);
      setFormData(initialState);
      navigate("/crafts");
    }
  };
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div>
        ...
        >
          ...
          <option value="67159bf0c5f0b8a90eb3b0fb">Painting and Drawing</option>
      ...
      <div>
        <label htmlFor="premium-membership">Require Premium Membership:</label>
        <select
      ...
      <div>
        ...
        <input
          ...
          name="step"
          min="1"
          placeholder={`Enter step ${directions.length + 1}`}
          defaultValue={directions.length + 1}
        />
      </div>
      <div>
